...
# This script disables, deletes, and/or archives on the local system.
...
# Make sure the script is being executed with superuser priviligies.
if [[ "${UID}" -ne 0 ]]
then
...
# Remove the options while le{aving the remaining arguments.
shift "$(( OPTIND -1 ))"
...
do
  ...
  then
    ...
    then
      ...
      echo "${HOME_DIR} does no exist or is not a directory." >&2
      exit 1
    fi
  fi
  ...
  then
    ...
    # Check to see if the userdel command succedded.
